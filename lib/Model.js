...
class Model extends EventEmitter {
    ...
    return Promise.resolve().then(() => {
      ...
      options.properties =
          ...
          (Array.isArray(options.properties) && !options.properties.length)
              ? this.getDataFields() : options.properties;
      ...
      delete opts.scope;
      return (new Finder(opts)).execute(options.scope);
    });
  }
  ...
  create(values, options) {
    return Promise.resolve().then(() => {
      ...
      const silent = options.silent != null ?
          options.silent : this.orm.options.silent;
      values = normalizePostValues(this, values, silent);
      ...
      return dbobj
          ...
          .execute({
            ...
            const x = returning || {};
            x.silent = silent;
            x.showSql = options.showSql;
            x.context = options.context;
            return this._responseReturning(dbobj, resp, x);
  ...
  update(values, options) {
    return Promise.resolve().then(() => {
      ...
      const silent = options.silent != null ?
          options.silent : this.orm.options.silent;
      values = normalizePostValues(this, values, silent, true);
      const returning = options.returning &&
          this._prepareReturning(options.returning, silent);
  ...
  _prepareReturning(value, silent) {
    ...
    for (const alias of Object.getOwnPropertyNames(properties)) {
      ...
      const field = typeof fname == 'string' && this.getField(fname, true);
      if (!field) continue;
  ...
  _responseReturning(dbobj, resp, options) {
    ...
    let needFind;
    const result = {};
    for (const attr of Object.getOwnPropertyNames(options.properties)) {
