...
const {ModelFindOne, ModelFindAll} = require('./model/find');
...
class Model {
   ...
   */
  find(options) {
    return new ModelFindOne(this, options);
  }
   ...
   */
  findAll(options) {
    return new ModelFindAll(this, options);
  }
  ...
  getAssociation(name) {
    const a = this._associations[name];
  ...
  _addAssociation(attribute, kind, options) {
    ...
    if (typeof options.model === 'string') {
      ...
      options = () => {
        opts.foreignModel = this.owner.get(opts.foreignModel);
        return opts;
      };
    }
