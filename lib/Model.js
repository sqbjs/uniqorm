...
class Model {
  ...
  get(keyValues, options, callback) {
    ...
    if (!callback)
      return promisify.fromCallback((cb) => this.get(options, cb));
  ...
  list(options, callback) {
    ...
    const opts = this._prepareListOptions(options,
  ...
  destroy(keyValues, options, callback) {
    ...
    const opts = merge({}, options);
    opts.where = this._prepareWhereforKeys(keyValues);
  ...
  _prepareListOptions(options, silent) {
    ...
    const addAttribute = (target, key, value) => {
      if (typeof value === 'string' || value == null)
        ...
        target[key] = this._prepareListOptions({attributes: value}, silent);
      else if (isPlainObject(value))
        target[key] = this._prepareListOptions(value, silent);
    };
