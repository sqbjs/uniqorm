...
class Model extends EventEmitter {
   ...
   *
   * @param {*|{string: *}} keyValue Key field value(s)
  ...
  get(keyValue, options = {}) {
    return promisify(() => {
      const opts = merge({
        filter: this._prepareKeyValues(keyValue),
        limit: 1
   ...
   * @param {Object|Array<Object>} [options.filter]
  ...
  update(values, options = {}) {
    return promisify(() => {
      ...
      let filter;
      if (options.where) {
        filter = this._mapConditions(options.where);
        filter = Array.isArray(filter) ?
            /* istanbul ignore next */ filter : [filter];
      } else {
        filter = [{}];
