...
class Model extends EventEmitter {
  ...
  constructor(orm, def) {
    ...
    this._orm = orm;
    this._associations = new AssociationSet(this);
    ...
    if (def.associations) {
      ...
      for (const n of def.associations)
        this.associations.add(n);
    }
  ...
  get(keyValues, options) {
    ...
    const getEvents = this.listeners('get');
    if (getEvents.length) {
  ...
  find(options) {
    ...
    return (new FindContext(opts)).execute();
  }
  ...
  _prepareFindOptions(options, silent) {
    ...
    result.autoCommit = options.autoCommit;
    return result;
  }
