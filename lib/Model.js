  ...
  find(options) {
    return Promise.resolve().then(() => {
      ...
      const silent = options.silent != null ?
          options.silent : this._orm.options.silent;
  ...
  create(values, options) {
    return Promise.resolve().then(() => {
      ...
      if (options.returning) {
        attributes = options.returning === '*' ? this.getDataFields() :
            (typeof options.returning === 'object' ? options.returning :
                [options.returning]);
      ...
      return query.execute({
        ...
        if (!(resp.rows && resp.rows.length && attributes))
          return true;

        let needFind;
        const result = {};
        for (const attr of Object.getOwnPropertyNames(attributes)) {
          const fieldName = attributes[attr] || attr;
