...
class Finder {
  ...
  _build() {
    ...
    this.connection.select()
        .where(...makeArray(callIf(this.filter, this.context)))
        .on('serialize', (fnd, type, o) => {
        ...
        joins.push(
            sqb.leftOuterJoin(
                ...
                .on(Op.eq('$$' + join.joinAlias + '.' +
                    ...
                    ...makeArray(callIf(join.ascField.filter, this.context)))
        );
      }
    }
    ...
    const processField = (finder, model, attrKey, v, tableAlias, targetNode) => {
      ...
      if (field.hasMany) {
        ...
        fnd = new Finder({
          ...
          silent,
          filter: [Op.in(field.foreignKey, new RegExp('__' + field.foreignKey)),
  ...
  _addJoin(ascField, parentAlias) {
    ...
    if (ascField.filter && ascField.filter.length)
      s += '|' + JSON.stringify(makeArray(callIf(ascField.filter, this.context)));
