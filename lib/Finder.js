...
class Finder {
      ...
      return query.execute({
        ...
        if (this._children) {
          for (const child of this._children) {
            ...
            const arr = child._values[prm] = makeArray(child._values[prm]);
        ...
        return this._executeChildren(scope).then(() => {
          ...
          for (const row of resp.rows) {
            ...
            if (isChild) {
              ...
              /* istanbul ignore else */
              if (key) {
                const arr = resultRows[key] = makeArray(resultRows[key]);
                arr.push(obj);
              }
            } else
              resultRows.push(obj);
  ...
  _build() {
    ...
    if (!this._masterColumn) // If not child finder
    ...
    /* 3. Prepare filter */
    const filter = this._processFilter(callIf(this.filter, this.context), 't');
  ...
  _processAttributes() {
    ...
    const processField = (finder, model, attrKey, v, tableAlias, targetNode) => {
      ...
      if (field.hasMany) {
        ...
        const masterCol = finder._addColumn(tableAlias, field.key);
        const foreignField = field.foreignModel.getField(field.foreignKey).fieldName;
        ...
        fnd = new Finder({
          ...
          silent,
          filter: [
            Op.in(tableAlias + '.' + foreignField,
                new RegExp('__' + field.foreignKey)),
            ...makeArray(callIf(field.filter, this.context))],
          sort: attrInfo.sort,
        ...
        fnd._masterColumn = masterCol.colName;
        fnd._masterAttr = attrKey;
        fnd._detailColumn =
            fnd._addColumn('t', field.foreignField.fieldName).colName;
        fnd._detailField = field.foreignKey;
  ...
  _addJoin(ascField, targetAlias) {
    ...
    if (join)
      return join;
    filter = this._processFilter(filter, joinAlias);
    join = {
      ascField,
      joinAlias,
      targetAlias,
  ...
  _processFilter(filter, tableAlias) {
    if (!filter)
      return [];
    if (filter.isOperator)
      return filter;
    if (Array.isArray(filter))
      return filter.map(x => this._processFilter(x, tableAlias));

    const result = {};
    for (const k of Object.keys(filter)) {
      const m = k.match(COLUMN_PATTERN);
      /* istanbul ignore next */
      if (!m)
        throw new ArgumentError('"%s" is not a valid field name', k);
      const fieldName = m[1];
      let field = this.model.getField(fieldName);
      if (field instanceof DataField) {
        result[tableAlias + '.' + k] = filter[k];
      } else {
        /* istanbul ignore next */
        ...
        if (field.fieldName)
          result[tableAlias + '.' + field.fieldName] = filter[k];
      }
    }
    return result;
  }
  ...
  _executeChildren(scope) {
    ...
    if (this._children) {
      ...
      for (const child of this._children) {
        const scp = scope ?
            scope.children[child._masterAttr] = {} : null;
        promises.push(child.execute(scp));
      }
