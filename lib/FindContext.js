...
class FindContext {
  ...
  _addAttributes(options) {
    ...
    const getAttrOptions = (attrKey, v) => {
      ...
      let result;
      const field = model.getField(v.field, silent);
      if (!field) return;
      ...
      if (field.foreignModel) {
        ...
        if (v.subField) {
          if (field.hasMany) {
            /* istanbul ignore next */
            if (silent) return;
            throw new ArgumentError('`%s` is an One2Many associated field and sub values can not be used to return as single value', v.field);
          }
          if (field.returnsSingleValue) {
            /* istanbul ignore next */
            if (silent) return;
            throw new ArgumentError('`%s` is an single value associated field and has no sub value `%s`', v.field, v.attribute);
          }
          if (!field.returnsAttribute(v.subField))
            throw new ArgumentError('`%s` has no attribute named `%s`', v.field, v.subField);
          ...
          if (v.attributes) {
            for (const n of Object.getOwnPropertyNames(v.attributes)) {
              if (!field.returnsAttribute(v.attributes[n] ||
                ...
                /* istanbul ignore next */
                if (silent) continue;
                throw new ArgumentError('`%s` has no attribute named `%s`', v.field, n);
              }
    ...
    Object.getOwnPropertyNames(options.attributes).forEach(attrKey => {
      ...
      while (fld) {
        ...
        if (fld.returnsSingleValue) {
          /* istanbul ignore else */
          if (fld.field) {
            node.column = ctx._addColumn(tableAlias, fld.field);
          }
        } else
