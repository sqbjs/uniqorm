    ...
    return Promise.resolve().then(() => {
      ...
      const modifyFieldName = (fieldName) => {
        ...
        switch (options.fieldNameFormat || 0) {
          case (NameFormat.CAMEL_CASE):
            n = camelCase(fieldName);
            break;
          case (NameFormat.CAPITALIZE_FIRST_LETTER):
            n = capitalizeFirst(fieldName);
            break;
        }
      ...
      const modifyModelName = (tableName) => {
        ...
        switch (options.modelNameFormat || 0) {
          case (NameFormat.CAMEL_CASE):
            n = camelCase(tableName);
            break;
          case (NameFormat.CAPITALIZE_FIRST_LETTER):
            n = capitalizeFirst(tableName);
            break;
        }
              ...
              .execute({
                ...
                for (const row of ret.rows) {
                  ...
                  associations.push({
                    ...
                    key: modifyFieldName(row.column_name),
                    foreignKey: modifyFieldName(row.foreign_column_name)
                  });
                }
              });
        }
...
function camelCase(str) {
  return str.toLowerCase().replace(/([-_].)/g, (x) => {
