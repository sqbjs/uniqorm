...
Model.extend = function (config) {
    ...
    assert(typeof name === 'string' && name.match(/([A-Za-z])\w*/), `Invalid model name "${name}"`);
    ...
    const
            ...
            constructor() {
                ...
                this._relations.forEach(relation => {
                    const t = relation.foreignModel.meta.tableName,
                        aa = t.substring(t, 1).toLowerCase();
                    let
                        a = aa, i = 1;
        ...
        meta = new ModelMeta(config);
    // static members
    Object.defineProperty(ctor, "name", {value: name || 'Model', writable: false, configurable: false});
    Object.defineProperty(ctor, "meta", {value: meta, writable: false, configurable: false});
    Object.defineProperty(ctor.prototype, "meta", {value: meta, writable: false, configurable: false});
    return ctor;
};
        ...
        const fields = new Proxy({}, {
            ...
            set: function (obj, key, value) {
                if (!(value instanceof Field)) {
                    const f = Field.get(value.type);
                    assert.ok(f, `Invalid field definition for ${key}`);
                    value = Reflect.construct(f, [value]);
                }
                assert.ok(value instanceof Field, `Invalid field definition for ${key}`);
